...
class EventsSelector extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            events_data:[],
            selected_machine : '',
            created_at__gte :formatDateForInput(new Date(new Date().setHours(new Date().getHours() - 8)).toString()), // get eight hours ago
            created_at__lte :formatDateForInput(new Date().toString()),
            machine:''
        }
        ...
        if(this.props.startLoading) this.props.startLoading()
            this.setState({...this.state, is_loading:true},async ()=>{
                try {
                    ...
                    if(this.props.onResult)this.props.onResult(response_events.data)
                  } catch (error) {
                        console.log(error)
    ...
    refreshData() {
        ...
        this.setState({...this.state, is_loading:true},async ()=>{
            try {
                ...
                if(!selected_machine) {
                    toast.error("Something wrong! Please try again. Hint: No machine created");
                  if(this.props.onResult)this.props.onResult([])
                    return
                }
                
                var filter_url = url + '/api/v2/events/?'
          ...
          <Card elevation={0}>
            <CardContent>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={3} md={3} display={'flex'} alignItems={'end'}>
                    <FormControl variant="standard" fullWidth sx={{ m: 1 }}>
                                    <Select
                                        id="machine"
                                        name='machine'
...
EventsSelector.contextType = AuthContext
